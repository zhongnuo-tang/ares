...
static int binary_update_download_binary( binary_update_info_t *binary_info, bool version_up, int condition )
{
    ...
    if ( strcmp( binary_info->name, "kernel" ) == 0 )
    {
        ...
        crc_hash = crc32part( (uint8_t *)&header_data.kernel_header + CHECKSUM_SIZE,
                              header_data.kernel_header.header_size,
                              crc_hash );
    }
    else if ( strcmp( binary_info->name, "common" ) == 0 )
    {
        ...
        crc_hash = crc32part( (uint8_t *)&header_data.common_header + CHECKSUM_SIZE,
                              header_data.common_header.header_size,
                              crc_hash );
    }
    else
    {
        ...
        crc_hash = crc32part( (uint8_t *)&header_data.user_header + CHECKSUM_SIZE,
                              header_data.user_header.header_size,
                              crc_hash );
    }
...
static void print_binary_info_list( binary_update_info_list_t *binary_info_list )
{
    ...
    for ( bin_idx = 0; bin_idx < binary_info_list->bin_count; bin_idx++ )
    {
        printf( " %4d | %6s | %8.1u | %8d\n",
                bin_idx,
                binary_info_list->bin_info[ bin_idx ].name,
                binary_info_list->bin_info[ bin_idx ].version,
...
static int
binary_update_check_test_result( binary_update_info_t *pre_bin_info, binary_update_info_t *cur_bin_info, int condition )
{
    int ret = ERROR;
