...
#define BLE_TEST_DUT_HANDLE (16)
#define BLE_TEST_SUT_HANDLE (24)
...
static void ble_test_scan_state_changed_cb(ble_scan_state_e scan_state)
{
	...
	test_scanning = scan_state;
	return;
}

static void ble_test_scanned_cb(ble_scanned_device *scanned_device)
{
	/* Compare Scanned device address */
	if (memcmp(scanned_device->addr.mac, test_scan_address, 6) == 0) {
		test_scanned_count++;
	}
}

static ble_result_e ble_create_connect_thread(void);
static void ble_test_device_disconnected_cb(ble_client_ctx *ctx)
{
	RMC_LOG(RMC_CLIENT_TAG, "'%s' is called[ID : %d]\n", __FUNCTION__, ctx->conn_handle);

	if ((!test_connecting) && (test_reconn_act)) {
		ble_client_destroy_ctx(ctx);
		ble_create_connect_thread();
	}
	return;
}

static void ble_test_device_connected_cb(ble_client_ctx *ctx, ble_device_connected *dev)
{
...
static void ble_test_operation_cb(ble_client_ctx *ctx, ble_attr_handle attr_handle, ble_data *read_result)
{
	if (read_result->length == sizeof(local_data)) {
		if(memcmp(read_result->data, local_data, (sizeof(local_data) - 1))) {
			RMC_LOG(RMC_CLIENT_TAG, "Error!! Data mismatch!!\n");
			RMC_LOG(RMC_CLIENT_TAG, "data : %x %x %x %x\n", read_result->data[0], read_result->data[1], read_result->data[2], read_result->data[3]);
		} else {
			if (test_recv_gatt_counter != read_result->data[3]) {
				RMC_LOG(RMC_CLIENT_TAG, "Warning!! Data mismatch!!\n");
				test_recv_gatt_counter = read_result->data[3];
			}
			RMC_LOG(RMC_CLIENT_TAG, "attr : %x // len : %d // data[3] : %x\n", attr_handle, read_result->length, read_result->data[3]);
			if (attr_handle == BLE_TEST_INDI_COMMAND) {
				test_recv_gatt_counter++;
			}
		}
	} else {
		RMC_LOG(RMC_CLIENT_TAG, "'%s' is called[%p]\n", __FUNCTION__, ctx);
...
static void ble_test_pair_bond_cb(ble_client_ctx *ctx, uint32_t bond_pair, ble_conn_handle con_handle)
{
	RMC_LOG(RMC_CLIENT_TAG, "'%s' is called[%p]\n", __FUNCTION__, ctx);
...
static ble_scan_callback_list test_scan_config = {
	ble_test_scan_state_changed_cb,
	ble_test_scanned_cb,
};
...
static void *ble_scan_result_thread(void *data)
{
	ble_result_e ret = BLE_MANAGER_FAIL;
	RMC_LOG(RMC_CLIENT_TAG, "- BLE Scan Result Thread-\n");
	RMC_LOG(RMC_CLIENT_TAG, "Start checking sacn result after 1mins\n");

ble_restart_scan:
	while (test_connecting) {											/* While connecting */
		sleep(5);														/* Wait */
	}
	sleep(3);

	ret = ble_client_set_scan(320, 160, 1);								/* scan_interval = 320;, scan_window = 160;, Active scan */
	RMC_LOG(RMC_CLIENT_TAG, "Scan Start without filter!\n");
	memcpy(test_scan_address, def_ext_addr_val, BLE_BD_ADDR_MAX_LEN);	/* Scan target address */
	ret = ble_client_start_scan(NULL, &test_scan_config);
	if (ret != BLE_MANAGER_SUCCESS) {
		RMC_LOG(RMC_CLIENT_TAG, "Warning!! Scan start fail[%d]\n", ret);
		goto ble_restart_scan;
	}
	test_scan_round_count = 0;

	while(1) {
		sleep(60);														/* Print the scanned result every 1min and reset the count */
		RMC_LOG(RMC_CLIENT_TAG, "Number of scanned address in 1min is: %d!\n", test_scanned_count);
		if (test_scanned_count == 0) {
			RMC_LOG(RMC_CLIENT_TAG, "Error!! Number of scanned address in 1min is abnormal!!!\n");
			RMC_LOG(RMC_CLIENT_TAG, "Error!! Number of scanned address in 1min is abnormal!!!\n");
			RMC_LOG(RMC_CLIENT_TAG, "Error!! Number of scanned address in 1min is abnormal!!!\n");
		}
		test_scan_round_count++;
		test_scanned_count = 0;
		if ((test_scan_round_count == 3) || (test_connecting)) {		/* Scanned for 3 round or connecting */
			RMC_LOG(RMC_CLIENT_TAG, "Scan stop\n");
			ret = ble_client_stop_scan();								/* Stop Scan */
			goto ble_restart_scan;
		}
	}
	return NULL;
...
static int ble_test_connect(ble_client_ctx *ctx, ble_addr *addr, bool is_auto)
{
	ble_result_e ret = BLE_MANAGER_FAIL;
	...
	conn_info.conn_interval = 96;		/* 120 ms (unit 1.25ms) */
	conn_info.slave_latency = 5;		/* 5 */
	conn_info.mtu = 240;
	...
	while (count--) {
		...
		if (cli_state == BLE_CLIENT_CONNECTED) {
			...
			break;
		} else if ((cli_state == BLE_CLIENT_IDLE) || (cli_state == BLE_CLIENT_NONE)) {
...
static void *ble_connect_thread(void *data)
{
	...
	RMC_LOG(RMC_CLIENT_TAG, "- BLE Connect Thread-\n");
	...
	ctx = ble_client_create_ctx(&test_client_config);			/* create ctx */
	if (ctx == NULL) {
		RMC_LOG(RMC_CLIENT_TAG, "Error!! create ctx fail\n");
		goto ble_conn_fail;
	}

ble_conn_restart:
	RMC_LOG(RMC_CLIENT_TAG, "Scan state: [%d]!\n", test_scanning);
	while (test_scanning) {										/* If still scanning */
		usleep(500 * 1000);										/* Wait 500ms */
	}
	RMC_LOG(RMC_CLIENT_TAG, "Try to connect!\n");
	...
	ret = ble_client_set_scan(160, 80, 1);						/* scan_interval = 160;, scan_window = 80;, Active scan */
	memcpy(test_scan_address, test_conn_addr_info.mac, BLE_BD_ADDR_MAX_LEN);
	ret = ble_client_start_scan(NULL, &test_scan_config);
	if (ret != BLE_MANAGER_SUCCESS) {
		RMC_LOG(RMC_CLIENT_TAG, "Warning!! Scan start fail[%d]\n", ret);
		goto ble_conn_restart;
	}
	test_scanned_count = 0;										/* Reset scanned count */
	while (test_scanned_count == 0) {							/* Havent found */
		sleep(1);
		test_scan_round_count++;
		if (test_scan_round_count == 30) {						/* Scanned for 30 round */
			ret = ble_client_stop_scan();						/* Stop Scan */
			if (ret != BLE_MANAGER_SUCCESS) {
				RMC_LOG(RMC_CLIENT_TAG, "Warning!! Scan stop fail[%d]\n", ret);
			}
			goto ble_conn_restart;
		}
		if (test_scanning == 0) {								/* Scan stopped */
			goto ble_conn_start;								/* To go connet */
		}
	}
	...
	RMC_LOG(RMC_CLIENT_TAG, "Found the Device!\n");
	while (test_scanning) {										/* If still scanning */
		usleep(500 * 1000);										/* Wait 500ms */
	}

ble_conn_start:
	ret = ble_test_connect(ctx, &test_conn_addr_info, 0);		/* Try to connect */
	...
	ble_manager_conn_is_active(BLE_TEST_DUT_HANDLE, &connection_active);	/* Client Role, always 16 */
	RMC_LOG(RMC_CLIENT_TAG, "%d active connection [%d]\n", BLE_TEST_DUT_HANDLE, connection_active);
	if (!connection_active)
		goto ble_conn_restart;									/* Retry connection */

	if (test_pairing_act) {										/* If pairing is enable */
		ret = ble_manager_start_bond(BLE_TEST_DUT_HANDLE);		/* DUT conn_handle is always 16 */
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_CLIENT_TAG, "Bond fail dev[%d]\n", ret);
		}
	}

	ret = ble_client_operation_enable_notification(ctx, BLE_TEST_NOTI_CCCD);
	if (ret != BLE_MANAGER_SUCCESS) {
		RMC_LOG(RMC_CLIENT_TAG, "Fail to enable notification [%d]\n", ret);
	}

	ret = ble_client_operation_enable_indication(ctx, BLE_TEST_INDI_CCCD);
	if (ret != BLE_MANAGER_SUCCESS) {
		RMC_LOG(RMC_CLIENT_TAG, "Fail to enable indication [%d]\n", ret);
	}

	test_connecting = 0;
	RMC_LOG(RMC_CLIENT_TAG, "- BLE Connect Thread Done! -\n");
	return NULL;

ble_conn_fail:
	test_connecting = 0;
	RMC_LOG(RMC_CLIENT_TAG, "Error!! BLE Connect Thread Fail!\n");
	return NULL;
}
...
static ble_result_e ble_create_connect_thread(void)
{
	...
	if (test_connecting) {
		RMC_LOG(RMC_CLIENT_TAG, "Already trying to connect!!\n");
		return BLE_MANAGER_ALREADY_WORKING;
	}
...
static ble_result_e ble_start_adv0(void)
{
	ble_result_e ret = BLE_MANAGER_FAIL;
	...
	RMC_LOG(RMC_SERVER_TAG, "Start adv [%d]\n", ret);
	return ret;
}
...
static void *ble_dut_test_thread(void *data)
{
	...
	/* TODO: DUT cannot be connected, so no need Adv 0*/
#if 0
	/* Slave:	Adv and connected by peer device, and continue adv even connected */
	/* 			Stay connected and seding data randomly */
	/* 			Disconnect at random time, and get reconnected again */
	ret = ble_start_adv0();										/* Start Adv0 */
	if (ret != BLE_MANAGER_SUCCESS) {
		RMC_LOG(RMC_SERVER_TAG, "Fail to start adv [%d]\n", ret);
		goto ble_dut_done;
	}
#endif
	uint32_t adv_interval_int[2] = { 288, 288 };				/* Interval 180ms */
	uint8_t adv_event_prop = 0x12;								//LE_EXT_ADV_LEGACY_ADV_SCAN_UNDIRECTED
	uint8_t adv_addr_type = 1;									//RTK_BT_LE_ADDR_TYPE_RANDOM
	uint8_t adv_addr[BLE_BD_ADDR_MAX_LEN] = { 0 };
	uint8_t adv_handle = 1;

	memcpy(adv_addr, def_ext_addr_val, BLE_BD_ADDR_MAX_LEN);	/* Use default Address */
	_reverse_mac(adv_addr, NULL);

	RMC_LOG(RMC_SERVER_TAG, "Unconnectable Adv with MAC [%02x:%02x:%02x:%02x:%02x:%02x]\n",
	...
	if (ble_create_connect_thread() != 0) {
		goto ble_dut_done;
	}

	RMC_LOG(RMC_CLIENT_TAG, "- Waiting for DUT connection -\n");
	...
	RMC_LOG(RMC_CLIENT_TAG, "- DUT connected -\n");					/* As Client, connected */

	/* Master role keep sending data candomly */
	uint8_t random_sleep;
	while(1) {
		random_sleep = rand() % 16;
		sleep(random_sleep);
	}

ble_dut_done:
	RMC_LOG(RMC_TAG, "Error!! BLE DUT Thread Error!!\n");
	return NULL;
}
...
static void *ble_sut_test_thread(void *data)
{
	...
	/* SUT, has 2 roles: Master and Slave */
	...
	pthread_t pid;
	if ((ret = pthread_create(&pid, NULL, (pthread_startroutine_t)ble_scan_result_thread, NULL)) != 0) {
		RMC_LOG(RMC_CLIENT_TAG, "pthread_create failed, status=%d\n", ret);
		goto ble_sut_done;
	} else {
		pthread_detach(pid);
...
ble_sut_check:
	sleep(5);
	bool connection_active;
	RMC_LOG(RMC_SERVER_TAG, "- Waiting for connection -\n");
	while (connection_active == 0) {
		sleep(3);
		ret = ble_manager_conn_is_active(BLE_TEST_SUT_HANDLE, &connection_active);		/* Server Role, always 24 */
		if (ret != BLE_MANAGER_SUCCESS) {
	...
	while(1) {
		...
		ble_manager_conn_is_active(BLE_TEST_SUT_HANDLE, &connection_active);		/* Server Role, always 24 */
		if (!connection_active) {								/* If connection is lost */
		...
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_SERVER_TAG, "Error!! Fail to send Notify[%d]!!\n", ret);
		}
...
ble_sut_done:
	RMC_LOG(RMC_TAG, "Error!! BLE SUT Thread Error!!\n");
	return NULL;
}
...
static uint8_t ctoi(char c)
{
	...
	RMC_LOG(RMC_TAG, "[%s]Error: Hex char is invalid !!!\r\n", __func__);
	return 0xFF;
}
...
bool hexdata_str_to_bd_addr(char *str, uint8_t *addr_buf, uint8_t buf_len)
{
	...
	if (str_len != 2 * BLE_BD_ADDR_MAX_LEN || buf_len < BLE_BD_ADDR_MAX_LEN) {
		RMC_LOG(RMC_TAG, "[%s]Error: Invalid bd addr string\r\n",__func__);
		return false;
	}
...
int ble_rmc_main(int argc, char *argv[])
{
	...
	*/
	...
	if (strncmp(argv[1], "advon", 6) == 0) {
		...
		/**
		 * Samsung Code
		 * Causes Warning on Compile.
		 * Not used in example, has been commented out.
	...
	if ((strncmp(argv[1], "noti_en", 8) == 0) || (strncmp(argv[1], "indi_en", 8) == 0)) {
		...
		if (ctx_count == 0) {
			RMC_LOG(RMC_CLIENT_TAG, "Not connected\n");
			goto ble_rmc_done;
		}
		for (uint8_t i = 0; i < RMC_MAX_CONNECTION; i++) {
			if ((ctx_list[i]) && (conn_handle == ctx_list[i]->conn_handle)) {
	...
	*/
	if (strncmp(argv[1], "test", 5) == 0) {
		...
		if (argc < 4) {
			RMC_LOG(RMC_CLIENT_TAG, "Test test_connecting: %d, test_scanning: %d\n", test_connecting, test_scanning);
			RMC_LOG(RMC_CLIENT_TAG, "Test test_scanned_count: %d, test_scan_round_count: %d\n", test_scanned_count, test_scan_round_count);
			RMC_LOG(RMC_CLIENT_TAG, "Error: Missing parameter, fail!!\n");
			goto ble_rmc_done;
		}
		...
		pthread_t pid;
		if (test_started) {
			RMC_LOG(RMC_CLIENT_TAG, "Test already started\n");
			goto ble_rmc_done;
		} 
		if (test_device_role) {
